package com.sztus.meshop.lib.core.util;
...
public class DateUtil {
    ...
    public static Integer diffDays(Date begin, Date end) {
        ...
        int tempDiffDays = BigDecimalUtil.div(
                diffTicks,
                DateUtil.MILLI_SECONDS_IN_DAY,
                0,
        ...
        beginCalendar.add(Calendar.DAY_OF_YEAR, tempDiffDays);

        if (endDays == beginCalendar.get(Calendar.DAY_OF_YEAR)) {
            return tempDiffDays;
        }

        beginCalendar.add(Calendar.DAY_OF_YEAR, -1);
        if (endDays == beginCalendar.get(Calendar.DAY_OF_YEAR)) {
            return tempDiffDays -1;
        }
        return tempDiffDays + 1;
    }
    ...
    public static long getTodayBeginningTimestamp() {
        return getLastDaysTimestamp(Calendar.getInstance(), 0);
    }
