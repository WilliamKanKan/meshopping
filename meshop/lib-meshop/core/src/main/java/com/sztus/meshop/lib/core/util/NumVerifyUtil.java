...
public class NumVerifyUtil {
    ...
    // 验证手机号码格式方法
    public static String phoneNumberMatcher(String phoneNumber) {
        String phoneNumberRegex = MatchFormat.PHONE;
        Pattern phoneNumberPattern = Pattern.compile(phoneNumberRegex);
        Matcher phoneNumberMatcher = phoneNumberPattern.matcher(phoneNumber);
        if (!phoneNumberMatcher.matches()) {
            return AjaxResult.failure("Incorrect phone number format");
        } else {
            return "success";
        }
    }
    ...
    public static final class MatchFormat{
        public static final String USERNAME = "^(?=(.*[a-zA-Z]){4})[a-zA-Z0-9]{6,}$";
